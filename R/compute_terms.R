...
#' @param dts_out a [matrix], the output of deSolve
#' @param pars a [list]
#' @param s the vector species index
#' @param i the host species index
#' @return [matrix]
#' @export
compute_terms <- function(varslist, dts_out, pars, s, i) {
  UseMethod("compute_terms", pars$compute)
}

#' @title Compute dynamical terms
#' @description Using the output of deSolve
#' compute the dynamical terms for the output of `dts_solve.ode` or `dts_solve.dde`
#' @inheritParams compute_terms
#' @return [list]
#' @export
compute_terms.dts <- function(varslist, dts_out, pars, s, i) {
  time = dts_out[,1]
  ...
  ni = list()
  for(i in 1:pars$nHosts){
    ni[[i]] = compute_NI(dts_out, pars, i)
...
#' @title Compute dynamical terms
#' @description Using the output of deSolve
#' compute the dynamical terms for the output of `dts_solve.cohort`
#' @inheritParams compute_terms
#' @return [list]
#' @export
compute_terms.cohort <- function(varslist, dts_out, pars, s, i) {
  time = dts_out[,1]
  d1 = length(time)
...
#' @title Compute dynamical terms
#' @description Using the output of deSolve
#' compute the dynamical terms for the output of `dts_solve.human`
#' @inheritParams compute_terms
#' @return [list]
#' @export
compute_terms.human<- function(varslist, dts_out, pars, s, i) {
  time = dts_out[,1]
...
#' @title Compute dynamical terms
#' @description Using the output of deSolve
#' compute the dynamical terms for the output of `dts_solve` that
#' don't have any relevant dynamical terms
#' @inheritParams compute_terms
#' @return [matrix]
#' @export
compute_terms.na <- function(varslist,dts_out, pars, s, i) {
 return(list())
}
...
#' @export
compute_NI <- function(dts_out, pars, i) {
  d1 = length(dts_out[,1])
  NI = sapply(1:d1, compute_NI_ix, dts_out=dts_out, pars=pars, i=i)
...
#' @export
compute_NI_ix <- function(ix, dts_out, pars, i) {
  t = dts_out[ix,1]
  y = dts_out[ix,-1]
  X <- F_X(t, y, pars, i)
  H = F_H(t, y, pars, i)
