...
# helper functions (not part of mosquito interface)

#' @title Calculate approximate equilibrium for RM model
#' @description Using a differential equation representation, calculate an approximate
#' equilibrium for the RM model. This is only relevant for mosquito models
#' constructed with [MicroMoB::setup_mosquito_RM], aquatic models setup with
#' [MicroMoB::setup_aqua_trace], and human models setup with [MicroMoB::setup_humans_SIS].
#' This function only sets state variables in the mosquito object, and `lambda`
#' in the aqua object.
#' @param model an object from [MicroMoB::make_MicroMoB]
#' @param X a vector of infectious human population sizes
#' @param H a vector of total human population sizes
...
#' @export
calculate_RM_equilibrium <- function(model, X, H, b, c, r, f, q, g, EIP) {

  stopifnot(inherits(model$mosquito, "RM"))
  stopifnot(inherits(model$aqua, "trace"))
  stopifnot(inherits(model$human, "SIS"))
  stopifnot(model$global$p == 1)

  # discrete time vs. continuous time semantics
  EIP_d <- EIP + 1
  ...
  Z <- (r*X*H) / (b*f*q*(H-X))
  Y <- Z / exp(-g*EIP_d)
  M <- (Z*(g + (f*q*kappa))) / (f*q*kappa*exp(-g*EIP_d))
  lambda <- g*M
  ...
  ZZ <- vapply(X = ZZ_tot, FUN = function(z) {
    z * pp
  }, FUN.VALUE = numeric(length(pp)))

  # set mosquito values
  model$mosquito$M <- M
  model$mosquito$Y <- Y
  model$mosquito$Z <- Z
  model$mosquito$ZZ <- ZZ

  # set lambda
  model$aqua$lambda <- matrix(data = lambda, nrow = 1, ncol = model$global$tmax)
