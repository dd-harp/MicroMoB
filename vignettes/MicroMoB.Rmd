...
A simplified conceptual overview of dynamics in **Micro-MoB** is shown below. Over
a time step, aquatic (immature) mosquitoes take in eggs laid by ovipositing adult mosquitoes
$G$, and give rise to newly emerging adults $\Lambda$. Adult mosquitoes undergo mortality
and search for blood meals. The proportion of bites on human hosts which cause infections
in mosquitoes is $\kappa$, and causes some fraction of mosquitoes to become infected $Y$
and eventually infectious $Z$. The human component takes in $EIR$, the per-capita rate
at which persons receive infectious bites, which affects prevalence $X$.

The red circle represents the _bloodmeal_, the process by which adult mosquitoes
initiate an oviposition cycle, seek out available hosts, and attempt to feed on them.
It is this process which transfers pathogens between the two species.
