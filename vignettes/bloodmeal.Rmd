...
In **Micro-MoB**, blood feeding is an algorithm which uses information provided by the 
...
Different colors refer to different components: green indicates human characteristics independent of infection status,
purple indicates human characteristics dependent upon infection status, blue indicates
...
The bloodmeal function updates several values within specific components. Those are:
  ...
  * $\kappa$: a vector of length `p`, giving the net infectiousness of hosts to mosquitoes in each patch.
  * $EIR$: a vector of length `n`, giving the entomological inoculation rate on hosts in each strata.
  * $f$: a vector of length `p`, giving the blood feeding rate of mosquitoes in each patch.
  * $q$: a vector of length `p`, giving the proportion of blood meals taken on human hosts by mosquitoes in each patch.
  
After these values have been computed and updates, each component's update (step) function
can appropriately update its own infection dynamics. These four values are the crucial
...
bloodfeeding an invariant, meaning any full MBPT model will need to simulate this process. 
...
## Net infectiousness of human population ($\kappa$)
...
\begin{equation}
\kappa_{p\times 1} = \upsilon \cdot (\beta^{\intercal} \cdot (xH)) + (1 - \upsilon) x_{\delta}
\end{equation}

## Directed wiring diagram

Another way to view the bloodmeal compuation is as a [directed wiring diagram (DWD)](https://www.algebraicjulia.org/blog/post/2021/01/machines/).
In this DWD there are two "boxes". Rectangles are elements of the model with state, similar to machines in automata theory.
Circles represent functions which do not have internal state, but may take some number of inputs and produce
