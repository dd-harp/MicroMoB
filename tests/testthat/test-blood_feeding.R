...
test_that("daily v. fractional beta, biting distribution matrix test", {
  ...
  # W_day <- (t(theta_day)) %*% (wf * H)
  # W_night <- (t(theta_night)) %*% (wf * H)
  ...
  beta_day <- compute_beta(human = model$human, t = 1)

  # calculate fractional  beta manually
  W <- t(theta[[1]]) %*% (wf * H)
  beta_day_manual <- diag(wf) %*% theta[[1]] %*% diag(1/as.vector(W))

  # problem(?)
  # beta_dt_day+beta_dt_night
  # ^ sums to beta_day*2 because the xi's cancel out (one in 1/W, one in Psi).
  # need to have folks take a look
